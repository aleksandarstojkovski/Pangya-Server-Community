...
| C | `CLIENT_COMET_REFILL` | `0xEC` | catch `0x197` u8 0 + 10 zeros |
...
| C | `CLIENT_ACTIVE_CUTIN` | `0xE5` | not-in-room CHANNEL catch silent |
...
| C | `CLIENT_PLAY_BIG_PAPEL_SHOP` | `0x186` | empty balls → `0x26C` `shopSys(0x5900103)` (same sys as normal Papel `0x14B`→`0x21B`) |
...
| C | `CLIENT_GZ_INITIAL` / marker / shot-end / chip-in / GZ first / wing / earcuff / glove / ring-ground / assist / Event Arin | `0x12D`/`0x12E`/`0x12F`/`0x131`/`0x137`/`0x138`/`0x171`/`0x180`/`0x181`/`0x184`/`0x185`/`0x192` | not-in-room CHANNEL catch silent |
...
| S | `SERVER_COMET_REFILL` | `0x197` | fail u8 0 + 10 zeros |
...
| S | `SERVER_EXTEND_RENTAL` | `0x18F` | fail u8 1 |
