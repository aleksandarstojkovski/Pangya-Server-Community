...
Fatto: Auth inbound `0x06` disconnect / `0x0C` confirm login / `0x0D` guild callbacks via `onAuthPacket`; Messenger `0x13` offline-userinfo stub; friend-list `canSeeOnline` (reverse-friend + guild-mate).
Prossimo opcode/file C#: Auth `0x0B` info-player-online; IFF pin/cube/`initComboDef`; capture JP S9.
