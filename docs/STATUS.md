...
Fatto: intrusion `0x9D` option 0 (`requestEnterGameAfterStarted`/`requestSendTimeGame`). Valida room Tourney pubblica, in-game e non piena; success `0x113` u8 3 + 0 + room + elapsed ms + time30s + RoomInfo 210. Invalid conserva u8 6 + u8 1. Schema resta **200** tabelle.
Prossimo opcode/file C#: UCC `0xB9`/`0xC9`; GZ first-hole `packet137` pulse; lucky-pouch `0xB2`.
...
- Parità client reale: ~169/197 Channel con happy-path; SQL al posto IFF; fail-stub su UCC/lucky-pouch; zero capture JP S9.
