...
S4 profondità: **148** opcode success 1:1 / **24** opcode solo fail-stub / **25** stimati rimanenti dal C# Channel
...
Fatto: club-set reset `0x16D` (`requestClubSetReset`) soft `0x1A000247` / hard `0x1A00024B`: SQL `iff_clubset` SlotStats + `tipo_rank_s` + `iff_clubset_rank_exp` (rank[] vuoto). Success persist consume+C/workshop reset (keep mastery) then `0x216` count 3 (type 2 + `0xCC` + `0xC9`) then `0x247` u32 0 + typeid + id. Hard also `0xC8` remaining+0. Catch CHANNEL `shopSys`; else full `0x5300500`. Mega `clientWorkshopTypeidClub(0x16D,0,0)` resta `shopSys(0x5300506)`. V21 (187 tabelle).
Prossimo opcode/file C#: workshop up-level `0x164` / rank `0x167`; oppure GZ first-hole `packet137` pulse; daily `0x152`–`0x154`; box-mail `0xEF`.
Blocco: file IFF assenti (pin/cube live, `initComboDef`, cutin success `0xE5` `findCutinInfomation`, workshop probability); nessuna capture client JP Season 9.
...
- Parità client reale: ~148/197 Channel con happy-path; SQL al posto IFF; fail-stub su workshop remaining/card/UCC/cutin/memorial; zero capture JP S9.
