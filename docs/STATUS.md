...
S4 profondità: **138** opcode success 1:1 / **34** opcode solo fail-stub / **35** stimati rimanenti dal C# Channel
...
Fatto: delete-item `0x64` (`requestDeleteActiveItem` / `pacote0C5`) per IFF ITEM (SQL warehouse C0 stand-in, no `findItem`/`IsItemEquipable`/giftable): success `0xC5` u8 1 + typeid + qntd + id; non-ITEM / missing / insufficient / truncated resta sbyte `-1`. Mega test `clientDeleteItem(1,1)` resta fail. Catch C# è sempre `-1`, non `shopSys`.
Prossimo opcode/file C#: GZ first-hole `packet137` pulse; oppure fail-stub IFF-free (daily `0x152`–`0x154`, workshop `0x164`–`0x16D`).
...
- Parità client reale: ~138/197 Channel con happy-path; SQL al posto IFF; fail-stub su workshop/card/UCC/cutin/memorial; zero capture JP S9.
