...
S4 profondità: **158** opcode success 1:1 / **14** opcode solo fail-stub / **15** stimati rimanenti dal C# Channel
...
Fatto: card-pack `0xCA` (`requestOpenCardPack`). SQL ordinato `card_pack_catalog` (V28, **194** tabelle) sostituisce `CardSystem.findCardPack/draws`; consuma una pack card, aggiunge le card estratte e manda `0x154` con row variabile C# (pack tail u8 draw-count, card tail u32 1). Catch sempre u32 1 come client-compatible C#.
Prossimo opcode/file C#: daily `0x152`–`0x154`; UCC `0xB9`; memorial `0x17F`; Tiki success `0x128`/`0x129`/`0x18D`; GZ first-hole `packet137` pulse; GP enter `0x179`.
...
- Parità client reale: ~158/197 Channel con happy-path; SQL al posto IFF; fail-stub su UCC/memorial/daily/Tiki/GP; zero capture JP S9.
