...
Fatto: attendance `0x16E`/`0x16F` (`requestCheckAttendance` / `requestUpdateCountLogin`, `pacote248`/`pacote249`) per SQL `pangya_attendance_table_item_reward` stand-in (no IFF `IsExist`): success i32 0 + u8 login + now + after + u32 counter; catch resta sempre u32 `~0`. Catalogo non seedato in V17 (mega `clientEmpty` resta fail). `0x16F` non manda mailbox GP/bot/fortune né achievement GUI. Opposite CLIENT locker add/remove `0x16E`/`0x16F`.
...
- Parità client reale: ~141/197 Channel con happy-path; SQL al posto IFF; fail-stub su workshop/card/UCC/cutin/memorial; zero capture JP S9.
