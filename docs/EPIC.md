...
| Campo | Valore |
|-------|--------|
| Fatto | club-set reset `0x16D` + SQL `iff_clubset` SlotStats/`tipo_rank_s`/`iff_clubset_rank_exp`: `0x216` count 3 then `0x247` u32 0; hard `0xC8` remaining+0; catch CHANNEL `shopSys` else `0x5300500` |
| Prossimo opcode/file C# | workshop up-level `0x164` / rank `0x167`, o GZ `packet137` pulse, daily `0x152`, box-mail |
...
| **S4** | Tutte le modalità C# + manager char/card/caddie/achievement | **non done** — Channel 197; ~148 success 1:1 / 24 fail-stub; IFF cubes/cutin/combo aperti |
...
- **175** tabelle in schema `pangya` (esclusi `temp_*` usati solo dentro stored proc) più **V8** `shop_catalog`, **V11** `cadie_magic_box`/`iff_caddie`/`iff_mascot`, **V12** `iff_card`, **V13** `iff_character_mastery`, **V14** `iff_character`/`iff_enchant`, **V15** `iff_card.efeito`, **V17** `iff_time_limit_item`, **V18** `iff_clubset`, **V19** `iff_part`, **V20** `iff_clubset` SlotStats e **V21** `iff_clubset_rank_exp`/`tipo_rank_s` (187).
