...
| Campo | Valore |
|-------|--------|
| Fatto | special card `0xBD` Effect=4: SQL `iff_card`; consume + Pang persist + `0x160` CardEquipInfo con date zero |
| Prossimo opcode/file C# | card-pack `0xCA`, daily `0x152`, UCC `0xB9`, o GZ `packet137` pulse |
...
| **S4** | Tutte le modalità C# + manager char/card/caddie/achievement | **non done** — Channel 197; ~157 success 1:1 / 15 fail-stub; IFF cubes/combo aperti |
