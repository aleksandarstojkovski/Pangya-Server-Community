...
| Campo | Valore |
|-------|--------|
| Fatto | workshop up-level `0x164`/`0x165`/`0x166` + SQL `iff_item`/`iff_clubset_level_up_*`/`total_recovery`: `0x216` then `0x23D`/`0x23E`/`0x23F`; catch CHANNEL `shopSys` else full |
| Prossimo opcode/file C# | workshop rank `0x167` / transform `0x168`–`0x169`, o GZ `packet137` pulse, daily `0x152`, box-mail |
...
| **S4** | Tutte le modalità C# + manager char/card/caddie/achievement | **non done** — Channel 197; ~151 success 1:1 / 21 fail-stub; IFF cubes/cutin/combo aperti |
...
- **175** tabelle in schema `pangya` (esclusi `temp_*` usati solo dentro stored proc) più **V8** `shop_catalog`, **V11** `cadie_magic_box`/`iff_caddie`/`iff_mascot`, **V12** `iff_card`, **V13** `iff_character_mastery`, **V14** `iff_character`/`iff_enchant`, **V15** `iff_card.efeito`, **V17** `iff_time_limit_item`, **V18** `iff_clubset`, **V19** `iff_part`, **V20** `iff_clubset` SlotStats, **V21** `iff_clubset_rank_exp`/`tipo_rank_s` e **V22** `iff_item`/`iff_clubset_level_up_limit`/`prob`/`total_recovery` (190).
