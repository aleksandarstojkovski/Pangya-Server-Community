...
| Campo | Valore |
|-------|--------|
| Fatto | club-set enchant `0x4B` opt 1/3 + SQL `iff_clubset` SlotStats/`iff_enchant`: `0xA5` kind+ClubSet+stat+id+pang; catch u8 0 |
...
| **S4** | Tutte le modalità C# + manager char/card/caddie/achievement | **non done** — Channel 197; ~147 success 1:1 / 25 fail-stub; IFF cubes/cutin/combo aperti |
