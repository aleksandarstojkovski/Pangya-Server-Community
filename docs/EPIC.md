...
| Campo | Valore |
|-------|--------|
| Fatto | rental extend/delete `0xE6`/`0xE7` PART+SQL `iff_part.valor_rental`: `0xC8`+`0x18F` / `0x190` u8 0; catch u8 1 |
...
| **S4** | Tutte le modalità C# + manager char/card/caddie/achievement | **non done** — Channel 197; ~146 success 1:1 / 26 fail-stub; IFF cubes/cutin/combo aperti |
...
- **175** tabelle in schema `pangya` (esclusi `temp_*` usati solo dentro stored proc) più **V8** `shop_catalog`, **V11** `cadie_magic_box`/`iff_caddie`/`iff_mascot`, **V12** `iff_card`, **V13** `iff_character_mastery`, **V14** `iff_character`/`iff_enchant`, **V15** `iff_card.efeito`, **V17** `iff_time_limit_item`, **V18** `iff_clubset` e **V19** `iff_part` (186).
