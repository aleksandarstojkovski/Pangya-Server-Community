...
| Campo | Valore |
|-------|--------|
| Fatto | item-buff `0xD8` ITEM+SQL TimeLimitItem: `0x181` u32 2 + count + typeid + ItemBuff; catch `0x5500400` |
| Prossimo opcode/file C# | GZ first-hole `packet137` pulse, o fail-stub IFF-free (daily `0x152`, workshop, box-mail) |
...
| **S4** | Tutte le modalità C# + manager char/card/caddie/achievement | **non done** — Channel 197; ~142 success 1:1 / 30 fail-stub; IFF cubes/cutin/combo aperti |
...
- **175** tabelle in schema `pangya` (esclusi `temp_*` usati solo dentro stored proc) più **V8** `shop_catalog`, **V11** `cadie_magic_box`/`iff_caddie`/`iff_mascot`, **V12** `iff_card`, **V13** `iff_character_mastery`, **V14** `iff_character`/`iff_enchant`, **V15** `iff_card.efeito` e **V17** `iff_time_limit_item` (184).
