...
| Campo | Valore |
|-------|--------|
| Slice completata | **S0–S6** packet paths: Versus replay `0x4A`→`0xA4` broadcast (Tourney `session_send`; consume warehouse C0; fail silent), Versus finish-shot `0x1C`→`0xCC` broadcast (Tourney `session_send`; empty cube list without IFF IDs; duplicate `finish_shot2` silent), use-item `0x17`→`0x5A` (catalog + ITEM group stand-in; Versus bans Mulligan Rose; fail silent), cutin fail `0xE5`→`0x18D` (Tourney/Versus u8 0+u16 1 without IFF; GZ u16 3), chip-in leave `0x131`→`0x1F2` + finish dump, shot-end `0x12F`→`0x1F7` (Versus turn oid/hole, Tourney sender, 87-byte echo), ring `0x15D`→`0x237`, glove `0x180`→`0x265`, earcuff `0x171`→`0x24C`, ring-ground `0x181`→`0x266`, rainbow/power/miracle/paws-set `0x196`→`0x27E` / `0x197`→`0x27F` / `0x198`→`0x280` / `0x199`→`0x281`, toggle assist `0x184`→`0x216`/`0x26A` (in-game reject `0x16A`), assist green `0x185`→`0x26B`, wing `0x138`→`0x203`, workshop count `0x173`→`0x24B`, Versus marker `0x12E`→`0x1F8`, paws `0x15C`→`0x236`, GP exit room `0x17A`→`0x254`, shop, messenger, lounge, GZ/SSC, lobby, chat, whisper, cookie, ready, change-room, player info `0x2F`, macros, GS list, rank, team, room detail, invite, leave+master `0x7C`, kick `0x26`, nick lookup `0xA1`, in-game camera/club/power/typing/drop, Versus load `0xA3`, whisper `0x55`, Match team chat `0xB0`, mailbox, last-5, daily quest `0x151`–`0x154`, delete-item fail `0xC5`, other-channel `0x83`, Cadie fail + Lolo fail/success, messenger list `0x8B`→`0xFC`, gacha `0x9E`→`0x102`, club-stats fail `0x4B`→`0xA5`, intrusion fail `0x9D`→`0x113`, Papel play `0x14B`→`0x216`/`0xFB`/`0x21B` + big `0x186`→`0xC8`/`0x216`/`0xFB`/`0x26C`, web-key `0xFB`→`0x1AD`, change-GS `0x119`→`0x1D4`/`0x9F`, ticket-report fail `0xAB`→`0x11A`, Tiki open `0x126`→`0x1E7`, locker `0xCC`/`0xD3`/`0xCD`/`0xD5`/`0xCE`/`0xCF`/`0xD0`–`0xD2`/`0xD4`, workshop fail `0x164`–`0x169`/`0x16B`–`0x16D`/`0x172`, lucky-pouch fail `0xB2`→`0x129`, tutorial fail `0xAE`→`0x44`, Tiki pts/exchange `0x127`–`0x129`, item-buff `0xD8`, comet `0xEC`, mail-box `0xEF`, refuse-whisper `0xDE`, My Room `0xB5`→`0x12B` + enter `0xB7`→`0x168`/`0x12D`, card `0xBD`/`0xCA`, rental `0xE6`/`0xE7`, memorial `0x17F`, UCC `0xB9`/`0xC9`, attendance fail `0x16E`/`0x16F`, GP lobby/room `0x176`/`0x177`/`0x179`, big Papel `0x186`→`0x26C`, character/card/tiki `0x187`–`0x18D`, finish-game aliases `0xCB`/`0x12C`, Versus load-ok `0x11`→`0x9E`/`0x5B`/`0x53` + CONTINUE_GO `0x36`→`0x5B`/`0x63`, gift success `0x1F`→`0xC8`/`0x6A`, personal-shop listing/buy `0x7C`/`0x7D`→`0xEB`/`0xEC`/`0xED`, Cadie success `0x158`→`0x216`/`0x22F`, Lolo compose `0x155`→`0xC8`/`0x216`/`0x229`/`0x22A`, character mastery `0x187`→`0x216`/`0x26E`, character stats `0x188`/`0x189`→`0xC8`/`0x216`/`0x26F` and `0x216`/`0x270`, character card `0x18A`–`0x18C`→`0x216`/`0x271`/`0x272`/`0x273`, caddie-holiday `0x39`→`0x93` u8 2, mascot message `0x73`→`0xE2` u8 4, Versus timeout `0x22`/`0x14`→`0x5C`, GM visible `0x8F`→`0x46`/`0x40`, GM whisper/channel/weather/kick `0x8F`→`0x40`/`0x9E`, GM identity `0x8F`/`0x41`→`0x9A`/`0x46`, GM giveitem/goldenbell mailbox `0x210`, GM versus wind `0x8F`→`0x5B`, GM whisper-list/disconnect `0x8F` + chat spy `0x40`, spy enter `0x3E` locked-room password |
| Prossima | IFF cubes/pins/`initComboDef`; cutin success `0xE5` IFF `findCutinInfomation`; remaining GM cmds (`CCG_HELP`/`SETPRIZE`/etc. unimplemented in C# default throw); real JP client captures |
...
- **175** tabelle in schema `pangya` (esclusi `temp_*` usati solo dentro stored proc) più **V8** `shop_catalog`, **V11** `cadie_magic_box`/`iff_caddie`/`iff_mascot`, **V12** `iff_card`, **V13** `iff_character_mastery`, **V14** `iff_character`/`iff_enchant` e **V15** `iff_card.efeito` (183).
