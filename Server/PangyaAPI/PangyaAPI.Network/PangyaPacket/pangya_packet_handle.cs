            ...
            try
            {
                ...
                if (result.check)
                {
                    ...
                    if (_session.isCreated() && result.len > 0)
                    {
                        ...
                        var decryptedPackets = ToClientBuffer.getPackets(result._buffer, _session.m_key);
                ...
                if (result.check)
                {
                    if (session.isCreated() && result.len > 0) // result.len 0 geralmente é um "Keep-Alive" vazio
                    {
                        ...
                        else
                        {
                            ...
                            var decryptedPackets = ToClientBuffer.getPackets(result._buffer, session.m_key);
