                ...
                _session.m_pi.v_cei.ToList().ForEach(_el =>
{
    ...
    motion_item.Contains(_element)))
        ...
        public void CreateRoomLogSql(RoomInfoLog _info)
        {
            ...
            {
                _smp.message_pool.getInstance().push(new message("[RoomGrandPrix::UpdateRoomLogSql][Log] not actived", type_msg.CL_FILE_LOG_AND_CONSOLE));
                return;
            }
     ...
     m_ri.qntd_hole == 18 &&
     m_ri.special_flag_mod.short_game == false &&
