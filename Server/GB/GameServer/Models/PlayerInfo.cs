...
namespace Pangya_GameServer.Models
{
 ...
 + ", POSITION=" + (cmd_igpc.getInfo().position) + "] novo com sucesso.", type_msg.CL_FILE_LOG_AND_CONSOLE));
        ...
        /// Natural = 903,
        /// GrandPrix = 903,
        /// All Statistic = 903*9(seasons) = 8127
        /// </summary>
        /// <returns>Bytes Write -> 10836 Size</returns>
        public byte[] GetMapStatistic()
        {
            using (var p = new PangyaBinaryWriter())
            {
                // Map Statistics Normal, Write 903 Bytes
                for (byte st_i = 0; st_i < MS_NUM_MAPS; st_i++)
                    p.WriteBytes(a_ms_normal[st_i].ToArray());

                // Map Statistics Natural, Write 903 Bytes
                for (byte st_i = 0; st_i < MS_NUM_MAPS; st_i++)
                    p.WriteBytes(a_ms_natural[st_i].ToArray());

                // Map Statistics Grand Prix, Write 903 Bytes
                for (byte st_i = 0; st_i < MS_NUM_MAPS; st_i++)
                    p.WriteBytes(a_ms_grand_prix[st_i].ToArray());

                // Map Statistics Normal for all seasons, Write 8127 Bytes
                for (int j = 0; j < 9; j++)//9 is number seasons game
