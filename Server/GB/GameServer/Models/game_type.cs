...
namespace Pangya_GameServer.Models
{
    ...
    public class ShotSyncData
    {
        ...
        public byte[] ShotDecrypt = new byte[16]; // Flag(valor) de penalidade do Grand Prix quando tem regras com penalidades
        public ShotSyncData(PangyaBinaryReader reader)
        { ToRead(reader); }
        public void ToRead(PangyaBinaryReader reader)
        {
            ...
            state = (SHOT_STATE)reader.ReadByte();
            ...
            state_shot = new stStateShot()
            {
                display = new stStateShot.uDisplayState()
                {
                    ulState = reader.ReadUInt32()
                },
                shot = new stStateShot.uShotState()
                {
                    ulState = reader.ReadUInt32()
                }
            };
            ...
            ShotDecrypt = reader.ReadBytes(16);
        }
        public byte[] ToArray()
        {
            using (var p = new PangyaBinaryWriter())
    ...
    public class PlayerGameInfo
    {
        ...
        public byte finish_hole = 0; // Usa no Grand Prix, PCBangMascot de sincronização de hole conluído para trocar para o prox

        public byte finish_hole2 = 0; // Usa no Grand Prix, PCBangMascot de sincronização do tempo do hole do player, para não dá time out depois que ele concluiu o hole

        public byte finish_hole3 = 0; // Usa no Grand Prix, PCBangMascot de sincronização se o player já enviou o pacote de finalizar o hole antes
