...
namespace Pangya_GameServer.Models
{
    ...
    public static class DefineConstants
    {
        ...
        public const int MS_NUM_MAPS = 21; // TH, US, KR is 20
   ...
   public class ctx_UCCWebKey
    ...
    public class MemberInfo
    {
        ...
        public string sComment;                // 0x4C: Comentário do jogador (32 bytes)
        public uint school;
        [field: MarshalAs(UnmanagedType.Struct)]
        public uCapability capability;
        public uint galleryUid;//galleryUid
        public int oid;
        ...
        public BlockFlag flag_block { get; set; }      //é 32 bytes é time_block, mas no Fresh UP JP o PCBangMascot block do pacote principal é de 64, então não tem mais o time block
        ...
        public byte[] ToArray()
        {
            using (var p = new PangyaBinaryWriter())
            {
                ...
                p.WriteUInt32(galleryUid);//gallery m_uid, spectator -> uid
                p.WriteInt32(oid);
                ...
                p.WriteByte(flag_login_time);     // 1 é primeira vez que logou, 2 já não é mais a primeira vez que fez login no server
                p.WriteBytes(papel_shop.ToArray()); //6
                //16
                p.WriteUInt32(point_point_event);         // S4 TH
                p.WriteUInt64(flag_block.m_id_state.ull_IDState);    //é 32 bytes é time_block, mas no Fresh UP JP o PCBangMascot block do pacote principal é de 64, então não tem mais o time block
                p.WriteUInt32(channeling_flag);         // S4 TH
                //
                p.WriteStr(sDisplayID, 128);   //
                Debug.Assert(!(p.GetSize != 261), "MemberInfo::Build() is Error");
                return p.GetBytes;
            }
        }
    }
    ...
    public class MemberInfoEx : MemberInfo
    {
        ...
        public byte[] ToArrayEx()
        {
            ...
            {
                p.Write(sala_numero);   //
                p.WriteBytes(message: ToArray());   //  261
                Debug.Assert(!(p.GetSize != 263), "MemberInfo::BuildEx() is Error");
                return p.GetBytes;
            }
        }
    }
    ...
    public class uCapability
    {
        ...
        public bool gallery //TODO: # 2 n�o sei bem mas estava na PCBangMascot que o mlk falou que era GM + PC Bang + Premium 
        {
    ...
    public class UserInfo
    {
       ...
       public float best_drive { get; set; }          // Max Distancia
        ...
        public int ladder_win { get; set; }             // TH S4
        public int ladder_lose { get; set; }            // TH S4
        public int ladder_draw { get; set; }            // TH S4
        public int ladder_hole { get; set; }            // TH S4
        ...
        public short skin_all_in_count { get; set; }
        public int event_value { get; set; }            // TH S4
        ...
        public byte[] ToArray()
        {
            using (var p = new PangyaBinaryWriter())
            {
                ...
                p.WriteFloat(best_drive);           // Max Distancia
                ...
                p.WriteFloat(best_long_putt);
                p.WriteFloat(best_chip_in);
                p.WriteUInt32(exp);
                p.WriteByte(level);
                p.WriteUInt64(pang);
                ...
                p.WriteInt32(ladder_win);               // TH S4
                p.WriteInt32(ladder_lose);          // TH S4
                p.WriteInt32(ladder_draw);          // TH S4
                p.WriteInt32(ladder_hole);          // TH S4
                p.WriteInt32(combo);
                ...
                p.WriteInt16(skin_all_in_count);
                p.WriteInt32(event_value);          // TH S4
        ...
        public override string ToString()
        {
            return "Tacada: " + (tacada) + "  Putt: " + (putt) + "  Tempo: " + (tempo) + "  Tempo Tacada: " + (tempo_tacada)
                ...
                + ", " + (best_score[3]) + ", " + (best_score[4]) + "]  Event PCBangMascot: " + (event_flag)
                ...
                + "  Disconnect(MY): " + (1/*disconnect*/) + "  Jogados Disconnect(MY): " + (jogados_disconnect) + "  Event value: " + (event_value)
                + "  Skin Strike Point: " + (skin_strike_point) + "  Sistema School Serie: " + (1/*sys_school_serie*/)
    ...
    _16bit_nao_sei.ToString(),
        ...
        public void ToRead(packet p)
        {
            ...
            best_drive = p.ReadSingle(); // p.WriteFloat
            ...
            // best_score (SBytes) - 5 bytes
            best_score = p.ReadSBytes(5);

            event_flag = p.ReadByte();

            // best_pang - No Writer você passou o objeto/array. 
            // Para manter a consistência com o Writer de 5 elementos:
            for (int i = 0; i < 5; i++)
            ...
            skin_all_in_count = p.ReadInt16();
            event_value = p.ReadInt32();
            jogados_disconnect = p.ReadInt32();
            game_count_season = p.ReadInt32();

            // medal - Aqui é onde os bytes costumam variar. 
            // Se deu 239 total, medal.ToArray() provavelmente resulta em 9 bytes:
            medal.lucky = p.ReadByte();
            medal.fast = p.ReadByte();
            medal.best_drive = p.ReadByte();
            medal.best_chipin = p.ReadByte();
            medal.best_puttin = p.ReadByte();
            medal.best_recovery = p.ReadByte();
    ...
    public class stMedal
    {
        ...
        public byte[] ToArray()
        {
            ...
            {    
                ...
                p.WriteByte(lucky);   // só tem no JP
                p.WriteByte(fast);//state_flag.ucByte(falta saber das flags)
                p.WriteByte(best_drive);     // 1 é primeira vez que logou, 2 já não é mais a primeira vez que fez login no server
                p.WriteByte(best_chipin);
                p.WriteByte(best_puttin);
                p.WriteByte(best_recovery);
                return p.GetBytes;
            }
        }
    ...
    public class MyRoomItem
    {
        ...
        public class Location
        {
            ...
            public void SetLoc(PangyaAPI.IFF.GB.Models.Data.Furniture.Location location)
            {
                x = location.x;
                y = location.y;
                z = location.z;
    ...
    public class TradeItem
    {
        ...
        public TradeItem(TradeItem item)
        {
            ...
            sd_name = string.Copy(item.sd_name ?? "");
            sd_copier_nick = string.Copy(item.sd_copier_nick ?? "");
        }
    ...
    public class stItem
    {
        ...
        public bool IsUCC()
        {
            return type_iff == (byte)PangyaAPI.IFF.GB.Models.Flags.PART_TYPE.UCC_DRAW_ONLY || type_iff == (byte)PangyaAPI.IFF.GB.Models.Flags.PART_TYPE.UCC_COPY_ONLY;
        }
        ...
        public short STDA_C_ITEM_QNTD { get => c[0]; set => c[0] = value; } //nao tem item é 0xFFFF
        ...
        //meu codigo
        public int STDA_C_ITEM_QNTD32 { get => Convert.ToInt32(STDA_C_ITEM_QNTD); }
        public int STDA_C_ITEM_TIME32 { get => Convert.ToInt32(STDA_C_ITEM_TIME); }
    }
	...
	**/
    ...
    {
        public const int SIZE_STRUCT = 348;
        ...
        public class StateAction
        {
            ...
            public short usUnknown_flg;//Unknown1	// Acho que seja uma PCBangMascot tbm
        ...
        public class stLocation
        {
            ...
            public float r { get; set; } // rotation
        ...
        public uint ulUnknown_flg;// Pode ser a PCBangMascot de teasure do player, ou de drop item
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 128)]
        private byte[] nick_NT_bytes;
        public string sDisplayID
        {
        ...
        public byte convidado;   // Flag Convidado, [Não sei bem por que os que entra na sala normal tem valor igual aqui, já que é PCBangMascot de convidado waiting]
        public float avg_score;// Media score "media de holes feito pelo player"
        public ushort ucUnknown3;// Não sei mas sempre é 0 depois do media score(66 no th)  
        ...
        public byte[] ToArray()
        {
            ...
            {
                p.WriteInt32(oid);
                p.WriteStr(nickname ?? "nick", 22);
                p.WriteStr(guild_name ?? "guildname", 17);
                p.WriteByte(position);
                p.WriteInt32(capability.ulCapability);
                p.WriteUInt32(title);
                p.WriteUInt32(char_typeid);
                p.WriteUInt32(skin);//array uint[6]
                ...
                p.WriteInt32(guild_uid);
                p.WriteStr(guild_mark_img ?? "guildmark", 9); //[12]);
                p.WriteUInt32(guild_mark_index);
                p.WriteUInt32(uid);
                //---------Action
                p.WriteUInt32(state_action.state_lounge);
                p.WriteInt16(state_action.usUnknown_flg);
                p.WriteUInt32(state_action.state);
                //-----------

                //---------- Move-Position
                p.WriteFloat(location.x);
                p.WriteFloat(location.z);
                p.WriteFloat(location.r);
                //-----------

                //------------ SHOP
                p.WriteUInt32(shop.active);
                p.WriteStr(shop.name ?? "shopping", 64);
                //------------    
                p.WriteUInt32(mascot_typeid);
                p.WriteUInt16(flag_item_boost.ulItemBoost);
                p.WriteUInt32(ulUnknown_flg);//333 bytes = 0x1400006C
                p.WriteStr(sDisplayID ?? "@NT", 128);//[22] Acho que seja o ID na ntreev do player, a empresa que mantêm as contas, no JP era o gamepot
                p.WriteByte(convidado);
                p.WriteFloat(avg_score);     
                p.WriteZero(2);
                return p.GetBytes;
            }
        }
    ...
    {
        public const int SIZE_STRUCT_EX = 854;
    ...
    public class RoomInfo
    {
        ...
        public enum INFO_CHANGE : uint
        {
            ...
            STATE_FLAG,
            UNKNOWN,
            HOLE_REPEAT,//SHIFT_HOLENUM
            FIXED_HOLE,//SHIFT_HOLELOCK 
            ARTEFATO,
            NATURAL,
        }
        ...
        public byte[] snome = new byte[40];
        public string nome { get => snome.GetString(); set => snome.SetString(value); }// [64];
        ...
        public override string ToString()
        {
            ...
            sb.AppendLine($"  PCBangMascot = {flag}");
            ...
            sb.AppendLine($"  tipo_ex = {tipo_ex}");
            sb.AppendLine($"  artefato = {artefato}");
    ...
    public class RoomGuildInfo
    {
        ...
        public byte[] ToArray()
        {
            using (var p = new PangyaBinaryWriter())
            {
                p.Write(guild_1_uid);
                p.Write(guild_2_uid);
                //p.WriteInt32(guild_1_index_mark);
                //p.WriteInt32(guild_2_index_mark);
                p.WriteStr(guild_1_nome, 17);
                p.WriteStr(guild_2_nome, 17);    
    ...
    public class RoomGrandPrixInfo
    {
        ...
        public byte[] ToArray()
        {
            using (var p = new PangyaBinaryWriter())
            {
                p.WriteUInt32(dados_typeid);
                p.WriteUInt32(rank_typeid);
                p.WriteUInt32(tempo);
                p.WriteUInt32(active);
                return p.GetBytes;
            }
        }
    ...
    public class RoomInfoEx : RoomInfo
    {
        ...
        public byte[] ssenha = new byte[40];
        public string senha { get => ssenha.GetString(); set => ssenha.SetString(value); } // Senha da sala
        ...
        public byte[] ToArray()
        {
            using (var p = new PangyaBinaryWriter())
            {
                p.WriteStr(nome, 40);
                p.WriteStr(senha, 24);
                p.WriteByte(senha_flag);
                p.WriteByte(state);
                p.WriteByte(flag);
                p.WriteByte(max_player);
                p.WriteByte(num_player);
                p.WriteBytes(key, 16);//a chave ficou errada
                p.WriteByte(gallery_num);
                p.WriteByte(_30s);
                p.WriteByte(qntd_hole);
                p.WriteByte(tipo_show);
                p.WriteUInt16(numero);
                p.WriteByte(modo);
                p.WriteByte(getMap());
                p.WriteUInt32(time_vs);
                p.WriteUInt32(time_30s);
                p.WriteUInt32(trofel);
                p.WriteInt16(state_flag);

                //----- GUILDS
                p.WriteBytes(guilds.ToArray());
                //----

                p.WriteUInt32(rate_pang);
                p.WriteUInt32(rate_exp);

                //p.WriteByte(flag_gm);//estava aqui....
                p.WriteInt32(master);
                p.WriteByte(tipo_ex);
                p.WriteUInt32(artefato);

                //----- UNaturalAndShortGame
                p.WriteUInt32(natural.ulNaturalAndShortGame);

                //----- GrandPrix
                p.WriteBytes(grand_prix.ToArray());
        ...
        public byte[] ToArrayEx()
        {
            using (var p = new PangyaBinaryWriter())
            {
                ...
                p.WriteByte(qntd_hole);
                p.WriteByte(modo);
                if (hole_repeat > 0 || getModo() == eMODO.M_REPEAT)
                {
                ...
                p.WriteUInt32(natural.ulNaturalAndShortGame);
                p.WriteByte(max_player);
                p.WriteByte(_30s);        // constante 30 de pangya
                p.WriteSByte((sbyte)state_flag);
                p.WriteUInt32(time_vs);
                p.WriteUInt32(time_30s);
                p.WriteUInt32(trofel);
    ...
    public class RoomInfoLog : RoomInfoEx
    {
        ...
        public uint Is_natural;           // Modo Natural ativo
        public uint Is_short_game;        // Short game ativo
        ...
        public uint enter_after_started;  // Entrou após o início
        public uint finish_game;          // Terminou o jogo
        ...
        public void UpdateInfo(uint _uid, int _character, int _club, int _mascot, int _caddie,
            uint _hole = 0, int _score = 0, uint _exp = 0, ulong _pang = 0,
            ulong _bonus_pang = 0, ulong _tacada_num = 0, ulong _total_tacada_num = 0, ulong _specialshot = 0,
            bool _premium = false, ulong _giveup = 0, uint _timeout = 0, uint _enter_after_started = 0,
            uint _finish_game = 0, uint _assist_flag = 0, uint _Win_trofeu = 0, uint _hithio = 0,
            uint _hitalba = 0, uint _hiteagle = 0, uint _hitbirdie = 0, uint _hitpar = 0,
            uint _hitbogey = 0, uint _hit_2_bogey = 0, uint _hit_3_bogey = 0, RoomInfoEx _ul = null)
        {
        ...
        public string ToString(bool isDb)
        {
            if (isDb)
            {
                return $"{nome}, {num_player}, {max_player}, {tipo_ex}, {uid}, {roomId}, {character}, {caddie}, {mascot}, {club}, {tipo}, {modo}, {qntd_hole}, {course}, {hole}, {score}, {exp}, {pang}, {bonus_pang}, {tacada_num}, {total_tacada_num}, {giveup}, {timeout}, {enter_after_started}, {finish_game}, {assist_flag}, {Win_trofeu}, {master}, {Is_short_game}, {Is_natural}, {HitHio}, {HitAlba}, {HitEagle}, {HitBirdie}, {HitPar}, {HitBogey}, {Hit_x2_Bogey}, {Hit_x3_Bogey}";
            }
            else
            {
    ...
    public class UserEquip
    {
        ...
        public uint m_title { get => skin_typeid[5]; set => skin_typeid[5] = value; }// Titulo Typeid 
    ...
    public class MascotInfo
    {
        ...
        public byte[] ToArray()
        {
            ...
            {                    
                ...
                p.WriteByte(PCBangMascot);

                return p.GetBytes;
            }   
        }
    }
    // Mascot Info Ex, tem o IsCash PCBangMascot nele
    [StructLayout(LayoutKind.Sequential, Pack = 1)]
    ...
    public class WarehouseItem
    {
        ...
        public bool IsUCC()
        {
            return sIff.getInstance().getItemGroupIdentify(_typeid) == sIff.getInstance().PART &&
    ...
    public class CardEquipInfo
    {
        ...
        public byte[] ToArray(bool build = true)
        {
            using (var p = new PangyaBinaryWriter())
            {
                ...
                p.WriteBuffer(value: use_date, 16);
                p.WriteBuffer(end_date, 16);
                if (build)
                {
                    p.WriteUInt32(tipo);
    ...
    public class ChannelInfo
    {
        ...
        public byte[] ToArray()
        {
            using (var Response = new PangyaBinaryWriter())
            {
                ...
                Response.WriteUInt32(flag.ulFlag); //ルーム制限あるね- channel PCBangMascot    
                Response.WriteUInt32(flag2); //ルーム制限あるね- channel PCBangMascot    
