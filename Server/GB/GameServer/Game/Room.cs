...
namespace Pangya_GameServer.Game
{
    ...
    public class Room : IDisposable
    {
        ...
        protected PlayerRoomInfoEx makePlayerInvitedInfo(Player _session)
        {
            ...
            pri.convidado = 1; // Flag Convidado, [Não sei bem por que os que entra na sala normal tem valor igual aqui, já que é PCBangMascot de convidado waiting], Valor constante da sala para os players(ACHO)
                ...
                && m_ri.master != -2 || (m_ri.getTipo() == RoomInfo.TIPO.GRAND_PRIX && m_ri.grand_prix.dados_typeid > 0))
            {
                ...
                else
                {
                    ...
                    if (new_trofel > 0 && new_trofel != m_ri.trofel)
                    {
                        ...
                        // Check se o trofeu anterior era o GM e se o novo não é mais, aí tira a PCBangMascot de GM da sala
        ...
        public virtual void finish_game()
        {
            ...
            if (m_pGame != null)
            {
                ...
                // Atualiza PCBangMascot da sala, só não atualiza se for GM evento ou GZ Event e SSC
                ...
                // Verifica se o Bot Tourney está ativo, kika bot e limpa a PCBangMascot
                if (m_bot_tourney)
                {
        ...
        public void updatePlayerInfo(Player _session)
        {
            ...
            try
            {
                ...
                if (m_ri.getTipo() == RoomInfo.TIPO.MATCH)
                {
                    ...
                    // atualizar o team do Player a PCBangMascot de team dele não bate com o team dele
                    ...
                    {
                        // a PCBangMascot de team do Player está errada, ele está no outro team, ajeita
                ...
                // Só atualiza a PCBangMascot de convidado se for diferente de 1, por que 1 ele é convidado
                if (pri.convidado != 1)
                {
        ...
        public void requestChatTeam(Player _session, packet _packet)
        {
            ...
            try
            {
                ...
                {
                    throw new exception("[room::requestChatTeam] [Error] PLAYER[UID=" + _session.m_pi.uid + "] tentou mandar messsage[MSG=" + msg + "] no chat do team na sala[NUMERO=" + m_ri.numero + "], mas ele nao esta no team que a PCBangMascot de team dele diz. Hacker ou Bug.", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.ROOM,
                        2004, 0));
                }
        ...
        public virtual bool requestStartGame(Player _session, packet _packet)
        {
            ...
            try
            {
                ...
                // Coloquei para verificar se a PCBangMascot de Bot tourney não está ativo verifica o resto das condições
                if (!m_bot_tourney
                    && v_sessions.Count() == 1
                ...
                rv.persist_rain = 0; // Persist rain PCBangMascot isso é feito na classe game 
        ...
        public void requestExecCCGChangeWeather(Player _session, packet _packet)
        {
            ...
            try
            {
                ...
                if (m_ri.getTipo() == RoomInfo.TIPO.LOUNGE)
                {
                    ...
                    p.WriteUInt16(m_weather_lounge);
                    p.WriteByte(1); // Acho que seja PCBangMascot, não sei, vou deixar 1 por ser o GM que mudou
