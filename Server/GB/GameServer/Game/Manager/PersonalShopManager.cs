    ...
    public partial class PersonalShopManager
    {
        ...
        public void openShop(Player _session, packet _packet)
        {
            ...
            try
            {
                ...
                if (count > 6)
                {
                    throw new exception("[PersonalShopManager::openShop][Error] PLAYER[UID=" + Convert.ToString(_session.m_pi.uid) + "] tentou abrir Personal Shop com um numero[value=" + Convert.ToString(count) + "] de itens é maior que o permitido", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.PERSONAL_SHOP_MANAGER,
                        250, 5200251));
                }
