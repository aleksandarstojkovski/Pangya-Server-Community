...
namespace Pangya_GameServer.Game.Manager
{
    ...
    public class RoomGrandPrix : Room, IDisposable
    {
...
#if _WIN32
				public CRITICAL_SECTION m_cs = new CRITICAL_SECTION();
#elif __linux__
				public pthread_mutex_t m_cs = new pthread_mutex_t();
#endif
            public bool m_state;
            public bool m_lock;
        }
                ...
                if (gp_condition != null)
                {
                    ...
                    switch (_cpir.type)
                    {
                        case ChangePlayerItemRoom.TYPE_CHANGE.TC_CADDIE:
                            if (grup_type == sIff.getInstance().CADDIE)
                            {
                                    ...
                                    && sIff.getInstance().getItemGroupIdentify(pCi._typeid) == sIff.getInstance().CADDIE)
                                {
                                    ...
                                    if (gp_condition.item_typeid != pCi._typeid)
                                    {
                                        ...
                                        if ((pCi = _session.m_pi.findCaddieByTypeid(gp_condition.item_typeid)) != null && sIff.getInstance().getItemGroupIdentify(pCi._typeid) == sIff.getInstance().CADDIE)
                                        {
                        ...
                        case ChangePlayerItemRoom.TYPE_CHANGE.TC_BALL:
                            if (grup_type == sIff.getInstance().BALL)
                            {
                                    ...
                                    && sIff.getInstance().getItemGroupIdentify(pWi._typeid) == sIff.getInstance().BALL)
                                {
                                    ...
                                    if (gp_condition.item_typeid != pWi._typeid)
                                    {
                                        ...
                                        if ((pWi = _session.m_pi.findWarehouseItemByTypeid(gp_condition.item_typeid)) != null && sIff.getInstance().getItemGroupIdentify(pWi._typeid) == sIff.getInstance().BALL)
                                        {
                        ...
                        case ChangePlayerItemRoom.TYPE_CHANGE.TC_CLUBSET:
                            if (grup_type == sIff.getInstance().CLUBSET)
                            {
                                WarehouseItemEx pWi = null;
                                    ...
                                    && sIff.getInstance().getItemGroupIdentify(pWi._typeid) == sIff.getInstance().CLUBSET)
                                {
                                    ...
                                    if (gp_condition.item_typeid != pWi._typeid)
                                    {
                                        ...
                                        if ((pWi = _session.m_pi.findWarehouseItemByTypeid(gp_condition.item_typeid)) != null && sIff.getInstance().getItemGroupIdentify(pWi._typeid) == sIff.getInstance().CLUBSET)
                                        {
                                            ...
                                            if (cs != null)
                                            {
                                                ...
                                                // ORIGINAL LINE: for (auto j = 0u; j < (sizeof(_session.m_pi.ei.csi.enchant_c) / sizeof(short)); ++j)
                        ...
                        case ChangePlayerItemRoom.TYPE_CHANGE.TC_CHARACTER:
                            if (grup_type == sIff.getInstance().CHARACTER)
                            {
                                    ...
                                    && sIff.getInstance().getItemGroupIdentify(pCe._typeid) == sIff.getInstance().CHARACTER)
                                {
                                    ...
                                    if (gp_condition.item_typeid != pCe._typeid)
                                    {
                                        ...
                                        if ((pCe = _session.m_pi.findCharacterByTypeid(gp_condition.item_typeid)) != null && sIff.getInstance().getItemGroupIdentify(pCe._typeid) == sIff.getInstance().CHARACTER)
                                        {
                        ...
                        case ChangePlayerItemRoom.TYPE_CHANGE.TC_MASCOT:
                            if (grup_type == sIff.getInstance().MASCOT)
                            {
                                MascotInfoEx pMi = null;

                                if (_cpir.mascot != 0
                                    && (pMi = _session.m_pi.findMascotById(_cpir.mascot)) != null
                                    && sIff.getInstance().getItemGroupIdentify(pMi._typeid) == sIff.getInstance().MASCOT)
                                {
                                    ...
                                    if (gp_condition.item_typeid != pMi._typeid)
                                    {
                                        ...
                                        if ((pMi = _session.m_pi.findMascotByTypeid(gp_condition.item_typeid)) != null && sIff.getInstance().getItemGroupIdentify(pMi._typeid) == sIff.getInstance().MASCOT)
                                        {
                        ...
                        case ChangePlayerItemRoom.TYPE_CHANGE.TC_ITEM_EFFECT_LOUNGE:
                            if (grup_type == sIff.getInstance().PART)
                            {
                        ...
                        case ChangePlayerItemRoom.TYPE_CHANGE.TC_ALL:
                            {
                                ...
                                else if (grup_type == sIff.getInstance().CADDIE)
                                {
                                    CaddieInfoEx pCi = null;
                                ...
                                else if (grup_type == sIff.getInstance().CLUBSET)
                                {
                                    WarehouseItemEx pWi = null;
                                ...
                                else if (grup_type == sIff.getInstance().BALL)
                                {
                                    WarehouseItemEx pWi = null;
        ...
        public override bool requestStartGame(Player _session, packet _packet)
        {
            ...
            try
            {
                ...
                if (sIff.getInstance().getGrandPrixAbaType(m_gp.ID) == GrandPrixData.GP_ABA.ROOKIE && !sIff.getInstance().isGrandPrixNormal(m_gp.ID))
                {
                    throw new exception("[RoomGrandPrix::requestStartGame][Error] PLAYER[UID=" + Convert.ToString(_session.m_pi.uid) + "] tentou comecar o jogo na sala[NUMERO=" + Convert.ToString(m_ri.numero) + "], mas a sala nao eh uma Grand Prix Rookie(Tuto). Hacker ou Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.ROOM_GRAND_PRIX,
                        1, 0x5900201));
                }
                ...
                else
                {
                    ...
                    if (m_ri.getMap() >= 0x7Fu)
                    {
                        ...
                        if (m_ri.getTipo() == RoomInfo.TIPO.SPECIAL_SHUFFLE_COURSE && m_ri.getModo() == RoomInfo.eMODO.M_SHUFFLE_COURSE)
                        {
                            ...
                            m_ri.course = (RoomInfo.eCOURSE)(0x80 | 17);
                    ...
                    rv.persist_rain = 0; // Persist rain PCBangMascot isso � feito na classe game
                    ...
                    switch (m_ri.getTipo())
                    {
                        case RoomInfo.TIPO.GRAND_PRIX:
                            ...
                            break;
                        default:
                            throw new exception("[RoomGrandPrix::requestStartGame][Error] PLAYER[UID=" + Convert.ToString(_session.m_pi.uid) + "] tentou comecar o jogo na sala[NUMERO=" + Convert.ToString(m_ri.numero) + ", MASTER=" + Convert.ToString(m_ri.master) + "], mas o tipo da sala nao eh Grand Prix. Hacker ou Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.ROOM_GRAND_PRIX,
                                9, 0x5900202));
                    }
        ...
        public bool startGame()
        {
            ...
            try
            {
                ...
                if (sIff.getInstance().getGrandPrixAbaType(m_gp.ID) == GrandPrixData.GP_ABA.ROOKIE && !sIff.getInstance().isGrandPrixNormal(m_gp.ID))
                {
                    throw new exception("[RoomGrandPrix::startGame][Error] Server tentou comecar o jogo na sala[NUMERO=" + Convert.ToString(m_ri.numero) + "], mas a sala nao eh uma Grand Prix Rookie(Tuto). Hacker ou Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.ROOM_GRAND_PRIX,
                        1, 0x5900201));
                }
