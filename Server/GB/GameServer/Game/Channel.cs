...
using PangyaAPI.IFF.GB.Models.Data;
using PangyaAPI.IFF.GB.Models.Flags;
    ...
    public class Channel
    {
                ...
                if (num_rooms > 0)
                {
                    ...
                    if (rooms.empty())
                    {
                        ...
                        if (_rt.m_type == range_time.eTYPE_MAKE_ROOM.TMR_MAKE_ALL || _rt.m_type == range_time.eTYPE_MAKE_ROOM.TMR_MAKE_INTERMEDIARE)
                        {
                            ...
                            // Flag do canal, se for rookie passa para sala, que no jogo, essa PCBangMascot faz vir vento de 1m a 5m
        ...
        public void requestMakeRoom(Player _session, packet _packet)
        {
            ...
            try
            {
                ...
                if (ri.tipo == 19 && len == 68)
                {
                    _packet.Skip(5);
                    ri.hole_repeat = 1;
                    ri.fixed_hole = 7;
                    ri.natural.ulNaturalAndShortGame = 0;
                    ri.modo = (byte)RoomInfo.eMODO.M_REPEAT;

                    practice = true;
                }
                //okay
                else if (ri.tipo == 19 && len == 63)
                {
                    ri.hole_repeat = 0;
                    ri.fixed_hole = 0;
                    practice = true;
        ...
        public void requestShowInfoRoom(Player _session, packet _packet)
        {
            try
            {
                ...
                for (var i = 0; i < v_session.Count; ++i)
                {
                    ...
                    // Cap do player, se for GM so mostra se estiver com a PCBangMascot visible
                    ...
                    case TPLAYER_ACTION.PLAYER_ACTION_LOUNGER_LOC: // X Z R, coordenada inicial do player no lounge
                        {
                            ...
                            _session.m_pi.location = new PlayerInfo.stLocation(location_add.x, location_add.z, location_add.r);
        ...
        public void requestEnterMyRoom(Player _session, packet _packet)
        {
            try
            {
                ...
                //pri.ucUnknown106
                pri.convidado = 0;  // Flag Convidado, [Não sei bem por que os que entra na sala normal tem valor igual aqui, já que é PCBangMascot de convidado waiting], Valor constante da sala para os players(ACHO)
        ...
        public void requestChangePlayerItemMyRoom(Player _session, packet _packet)
        {
            ...
            _smp.message_pool.getInstance().push(new message($"[{this.GetType().Name}::{MethodBase.GetCurrentMethod().Name}][Debug] Packet 0x20.\n\rHex Dump.\n\r" + _packet.Log(), type_msg.CL_FILE_LOG_AND_CONSOLE));
            ...
            try
            {
                ...
                switch (type)
                {
                    ...
                    case 5: // only Character ID EQUIPADO
                        {
                            CharacterInfo pCe = null;
                            item_id = _packet.ReadInt32();
                            pCe = _session.m_pi.findCharacterById(item_id);
                            if (pCe != null && sIff.getInstance().getItemGroupIdentify(pCe._typeid) == sIff.getInstance().CHARACTER)
                            {
                    ...
                    case 9: // Character Cutin
                        {
                            ...
                            item_id = _packet.ReadInt32();
                            pCe = _session.m_pi.findCharacterById(item_id);
                            if (pCe != null && (sIff.getInstance().getItemGroupIdentify(pCe._typeid) == sIff.getInstance().CHARACTER && _session.m_pi.ei.char_info != null && _session.m_pi.ei.char_info.id == pCe.id))
                            {
            ...
            try
            {
                ...
                if (qntd > 0)
                {
                    ...
                    for (var i = 0u; i < qntd; ++i)
                    {
                        ...
                        // BuyItem é a PCBangMascot de visivel no pangya shop
                    ...
                    if (coupon.id != 0 && coupon._typeid != 0u)
                    {
                        ...
                        p.WriteInt32(coupon.flag_time); // Time Tipo(ou PCBangMascot)
        ...
        public void requestCadieCauldronExchange(Player _session, packet _packet)
        {
            ...
            try
            {
                ...
                { // Random Item
                    ...
                    if (item_random.Shop.flag_shop.time_shop.active)
                    {
                        ...
                        else
                        {
                            ...
                            // Verifica aqui por questão de segurança, mas tem que ter a PCBangMascot no IFF_STRUCT de tempo com a quantidade de dias
                ...
                foreach (var el in v_remove)
                {
                    ...
                    p.WriteInt32(el.id);
                    p.WriteInt32(el.flag_time); // Time Tipo(ou PCBangMascot)
        ...
        public void requestOpenScratchCard(Player _session, packet _packet)
        {
            ...
            try
            {
                ...
                //CHECK_SESSION_IS_AUTHORIZED("requestOpenScratchCard");

                if (_session.m_pi.block_flag.m_flag.scratchy)
                    throw new exception("[game_server::requestOpenScratchCard][Error] Player[UID=" + (_session.m_pi.uid)
                        + "] esta bloqueado o Rank Server, ele nao pode acessar o requestOpenScratchCard.", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.GAME_SERVER, 7010, 0));
            ...
            catch (exception)
            {
                ...
                //_smp::message_pool::getInstance().push(new message("[game_server::requestOpenScratchCard][ErrorSystem] ", CL_FILE_LOG_AND_CONSOLE));
        ...
        public void requestPlayScratchCard(Player _session, packet _packet)
        {
            PangyaBinaryWriter p = new PangyaBinaryWriter();
            ...
            try
            {
                ...
                if (_session.m_pi.block_flag.m_flag.scratchy)
                    throw new exception("[channel::requestPlayScratchCard][Error] player[UID=" + (_session.m_pi.uid)
                        + "] tentou jogar no Scratch Card, mas ele nao pode. Hacker ou Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 3, 0x790001));
                ...
                var coupon = sScratchCardSystem.getInstance().hasCoupon(_session);

                if ((coupon == null || coupon.STDA_C_ITEM_QNTD < 1))
                    throw new exception("[channel::requestPlayScratchCard][Error] player[UID=" + (_session.m_pi.uid) + "] tentou jogar o Scratch Card, ele nao tem Coupon. Hacker ou Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 2, 0x5900102));

                var balls = sScratchCardSystem.getInstance().Play(_session);

                if (balls.empty())
                    throw new exception("[channel::requestPlayScratchCard][Error] player[UID=" + (_session.m_pi.uid) + "] tentou jogar o Scratch Card Normal, mas nao conseguiu sortear os items. Bug",
                        ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 3, 0x5900103));
                ...
                for (int i = 0; i < balls.Count; i++)
                {
                    ...
                    if (item._typeid == 0)
                        throw new exception("[channel::requestPlayScratchCard][Error] player[UID=" + (_session.m_pi.uid) + "] tentou jogar o Scratch Card Normal, mas nao conseguiu inicializar o Item[TYPEID="
                            + (bi._typeid) + "]. Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 4, 0x5900104));
                ...
                if (rai.fails.Count > 0 && rai.type != ItemManager.RetAddItem.T_SUCCESS_PANG_AND_EXP_AND_CP_POUCH)
                    throw new exception("[channel::requestPlayScratchCard][Error] player[UID=" + (_session.m_pi.uid) + "] tentou jogar o Scratch Card, mas nao conseguiu adicionar o(s) Item(ns){"
                        + ids + "}", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 6, 0x5900106));
                ...
                if (coupon != null)
                {
                    ...
                    WarehouseItemEx card_scratch = new WarehouseItemEx();

                    var it = _session.m_pi.findWarehouseItemByTypeid(coupon._typeid);
                    item.clear();
                    var qtnd = it.c[0];
                    item.type = 2;
                    item.id = coupon.id;
                    ...
                    item.STDA_C_ITEM_QNTD = (short)(item.qntd * 1);//novo?

                    if (ItemManager.removeItem(item, _session) <= 0)
                        throw new exception("[channel::requestPlayScratchCard][Error] player[UID=" + (_session.m_pi.uid) + "] tentou jogar o Scratch Card Normal, mas nao conseguiu deletar o Coupon[TYPEID="
                            + (coupon._typeid) + ", ID=" + (coupon.id) + "]. Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 5, 0x5900105));
                }
                else    // Não tem Coupon Tira Pangs do player
                {
                ...
                foreach (var el in v_item)
                {
                    ...
                    p.WriteInt32(el.id);
                    p.WriteUInt32(el.flag_time);
                    p.WriteInt32(el.stat.qntd_ant);    // qntd ant
                    p.WriteInt32(el.stat.qntd_dep);    // qntd dep
                ...
                // Resposta para o Play Scratch Card Normal
                p.init_plain(0xDD);
                p.WriteUInt32(0);       // OK
                p.WriteInt32(balls.Count);      // OK
                foreach (var el in balls)
                {
                    p.WriteUInt32(0);       // OK
                    p.WriteUInt32(el.ctx_psi._typeid);
        ...
        public void requestPlayScratchCardSerial(Player _session, packet _packet)
        {
            //    REQUEST_BEGIN("requestPlayScratchCardSerial");

            //    PangyaBinaryWriter p = new PangyaBinaryWriter();

            //    struct ScratchCardSerial
            //{
            //    ScratchCardSerial() { clear(); };
            //    void clear()
            //    {
            //        memset(this, 0, sizeof(ScratchCardSerial));
            //    };
            //    uint32_t Check_Size;
            //    std::string Serial;
            //    bool ValidSerial;
            ...
            //ScratchCardSerial scard_info = { };
            //try
            //{
            //    scard_info.Check_Size = _packet.readUint32();
            //    scard_info.ValidSerial = scard_info.Check_Size != 13;
            //    // Verifica se session está autorizada para executar esse ação, 
            //    // se ele não fez o login com o Server ele não pode fazer nada até que ele faça o login
            //    //CHECK_SESSION_IS_AUTHORIZED("requestPlayScratchCardSerial");

            //    if (_session.m_pi.block_flag.m_flag.stBit.scratchy)
            //        throw exception("[channel::requestPlayScratchCardSerial][Error_Block] player[UID=" + (_session.m_pi.uid)
            //            + "] tentou jogar no Scratch Card, mas ele nao pode. Hacker ou Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 3, 0x790001));

            //    if (scard_info.Check_Size < 13)
            //        throw exception("[channel::requestPlayScratchCardSerial][Error_Check_Size] player[UID=" + (_session.m_pi.uid)
            //            + "] tentou jogar no Scratch Card, mas ele nao pode. Hacker ou Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 3, 0x790001));

            //    scard_info.Serial = _packet.readString(scard_info.Check_Size);

            //    if (scard_info.Serial.empty())
            //        throw exception("[channel::requestPlayScratchCardSerial][Error_String] player[UID=" + (_session.m_pi.uid)
            //            + "] tentou jogar no Scratch Card, mas ele nao pode. Hacker ou Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 3, 0x790001));


            //    if (scard_info.Serial.size() != scard_info.Check_Size)
            //        throw exception("[channel::requestPlayScratchCardSerial][Error_Size] player[UID=" + (_session.m_pi.uid)
            //            + "] tentou jogar no Scratch Card, mas ele nao pode. Hacker ou Bug", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 3, 0x790001));

            //    // Resposta para o Play Scratch Card Serial
            //    p.init_plain(0xDE);
            //    p.WriteUInt32(0);       // OK
            //    p.WriteUInt32((uint32_t)3);// OK,return code 0 success, 1 used, 2 invalid, 3 expired etc...
            //    p.WriteUInt32((uint32_t)0);     // OK		
            //    packet_func.session_send(p, _session, 0);
            //    //falta mais resposta aqui :/
            //}
            //catch (exception)
            //{

            //    //_smp::message_pool::getInstance().push(new message("[channel::requestPlayScratchCardSerial][ErrorSystem] ", CL_FILE_LOG_AND_CONSOLE));

            //    p.init_plain(0xDE);

            //    p.WriteUInt32(371598848);

            //    packet_func.session_send(p, _session, 0);
            //}
        }

        public void requestChangeNickName(Player _session, packet _packet)
        {
            //    REQUEST_BEGIN("PlayerChangeNickName");

            //    PangyaBinaryWriter p = new PangyaBinaryWriter();

            //    enum NICK_CHECK : unsigned char {
            //		SUCCESS = 1,                // Sucesso por trocar o nick por que ele está disponível
            //        NICK_IN_USE = 2,            // NICKNAME já em uso
            //        HAVE_BAD_WORD = 3,          // Tem palavras que não pode no NICK
            //        EMPETY_ERROR = 4,           // Erro Vazio
            //        SAME_NICK_USED = 5,			// O Mesmo nick vai ser usado, estou usando para o mesmo que o ID
            //	};

            //NICK_CHECK code(SUCCESS);
            //try
            //{

            //    // Verifica se session está autorizada para executar esse ação, 
            //    // se ele não fez o login com o Server ele não pode fazer nada até que ele faça o login
            //    //CHECK_SESSION_IS_AUTHORIZED("PlayerChangeNickName");

            //    std::wstring wnick = _packet.readWString();
            ...
            //				wcscasecmp(
            //#endif
            //                std::wstring(_session.m_pi.nickname, _session.m_pi.nickname + sizeof(_session.m_pi.nickname)).c_str(), wnick.c_str()
            //				) == 0
            //			) {
            //        code = SAME_NICK_USED;    // NICK igual ao ID, não pode
            //        throw exception("NICK_OLD == NICK_NEW! ", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 1, 0));
            //    }

            //    // Palavras que não podem ser usadas
            //    if (code == SUCCESS && !(_session.m_pi.mi.capability.ulCapability & 4) && std::regex_search(wnick, std::wregex(L"(.*GM.*)|(.*ADM.*)", std::regex_constants::icase)))
            //    {
            //        code = HAVE_BAD_WORD;
            //        throw exception("HAVE BAD WORD! ", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 1, 0));
            //    }

            //    if (code == SUCCESS && std::regex_search(wnick, std::wregex(L".*[ ].*")))
            //    {
            //        code = EMPETY_ERROR;
            //        throw exception("HAVE BAD WORD! ", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 1, 0));
            //    }

            //    if (code == SUCCESS && (wnick.size() < 4 || std::regex_search(wnick, std::wregex(L"[\\^$&,\\?`´~\\|\"@#¨'%*!\\\\]"))))
            //    {
            //        code = NICK_IN_USE;
            //        throw exception("HAVE BAD WORD! ", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 1, 0));
            //    }

            //    if ((uint32_t)wnick.size() < 4 || (uint32_t)wnick.size() > 16)
            //    {
            //        code = NICK_IN_USE;
            //        throw exception(" size nick invalid, nick.size > 16", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 1, 0));

            //    }

            //    if (_session.m_pi.cookie < 7000)
            //    {
            //        if (_session.m_pi.mi.capability.stBit.game_master)
            //        {
            //            _session.addCookie(7000);
            //        }
            //        else
            //        {
            //            code = EMPETY_ERROR;
            //            throw exception(" Bug not have 7000 cookies", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 1, 0));
            //        }
            //    }
            //    auto new_nick = std::string(wnick.begin(), wnick.end());
            //    // Load Config
            //    CmdChangeNick cmd_change_nick(_session.m_pi.uid, new_nick, true); // Waiter

            //    snmdb::NormalManagerDB::getInstance().add(0, &cmd_change_nick, null, null);

            //    cmd_change_nick.waitEvent();

            //    if (cmd_change_nick.getException().getCodeError() != 0)
            //        throw cmd_change_nick.getException();

            //    code = NICK_CHECK(cmd_change_nick.getCode());
            //    if (code != SUCCESS && _session.m_pi.mi.capability.stBit.game_master == 0)
            //    {
            //        throw exception("Falied to change Nick", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.CHANNEL, 1, 0));
            //    }
            //    if (code == SUCCESS)
            //    {
            //        p.init_plain(0x50);
            //        p.WriteUInt32(0);
            //        p.addString(new_nick);
            //        packet_func::session_send(p, &_session, 0);
            //        STRCPY_TO_MEMORY_FIXED_SIZE(_session.m_pi.mi.nick_name, sizeof(_session.m_pi.mi.nick_name), new_nick.c_str());
            //        STRCPY_TO_MEMORY_FIXED_SIZE(_session.m_pi.nickname, sizeof(_session.m_pi.nickname), new_nick.c_str());
            //        //se não for gm ou A.I
            //        _session.m_pi.consomeCookie(7000);
            //        packet_func::pacote096(p, &_session, &_session.m_pi);
            //        packet_func::session_send(p, &_session, 0);
            //        updatePlayerInfo(_session);
            //        sendUpdatePlayerInfo(_session, 3);
            //    }
            ...
            //    p.WriteUInt32(code);
            //    packet_func::session_send(p, &_session, 0);
            //    //_smp::message_pool::getInstance().push(new message("[channel::requestChangeNickName][ErrorSystem] [Player= " + (_session.getUID()) + "] ", CL_FILE_LOG_AND_CONSOLE));
            //}
                    ...
                    if (_session.m_pi.mi.capability.game_master)      //comandos [player gm/adm]
                    {
                        if (r != null) //comandos em sala
                        {
                            ...
                            if (cmd == ("@weather") && (r.getInfo().getTipo() == RoomInfo.TIPO.MATCH || r.getInfo().getTipo() == RoomInfo.TIPO.STROKE || r.getInfo().getTipo() == RoomInfo.TIPO.TOURNEY || r.getInfo().getTipo() == RoomInfo.TIPO.LOUNGE) || r.getInfo().getTipo() == RoomInfo.TIPO.SPECIAL_SHUFFLE_COURSE || r.getInfo().getTipo() == RoomInfo.TIPO.PRACTICE || r.getInfo().getTipo() == RoomInfo.TIPO.GRAND_ZODIAC_PRACTICE)
                            {
                                ...
                                p.WriteByte(1);  // PCBangMascot ou indicação de GM
