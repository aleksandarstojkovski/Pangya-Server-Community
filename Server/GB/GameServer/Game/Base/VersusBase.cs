...
namespace Pangya_GameServer.Game.Base
{
    public abstract class VersusBase : GameBase, IDisposable
    {
        ...
        public virtual void sendPlayerTurn()
        {
            ...
            p.WriteByte(1); // Flag do vento, 1 Reseta o Vento, 0 soma o vento que nem o comando gm \wind do pangya original, , Também é PCBangMascot para trocar o vento no Pang Battle se mandar o valor 0
        ...
        public virtual void CCGChangeWind(Player _gm, byte _wind, ushort _degree)
        {
            ...
            try
            {
                ...
                p.WriteByte((wind_flag < 0) ? 1 : 0); // Card Wind Flag, minus wind PCBangMascot
                p.WriteUInt16(m_player_turn.degree); // Degree
                p.WriteByte(1); // Flag 1 = Reset Degree, 0 = Plus Degree, , Também é PCBangMascot para trocar o vento no Pang Battle se mandar o valor 0
        ...
        public virtual void init_turn_hole_start()
        {
            ...
            foreach (var el in m_players)
            {
                ...
                if (el != null)
                {
                    ...
                    if (pgi.flag == PlayerGameInfo.eFLAG_GAME.QUIT)
                    {
                        _smp.message_pool.getInstance().push(new message($"[VersusBase::init_turn_hole_start][Log] Jogador UID={pgi.uid} está com PCBangMascot QUIT, será ignorado.", type_msg.CL_FILE_LOG_AND_CONSOLE));
                    }
                    else
                    {
