...
namespace Pangya_GameServer.Game.Base
{
    public abstract class GameBase : IDisposable
    {
        ...
        {
            if (_buffer.Length < 54 || _buffer.Length > 54)
                return null;
            ...
            //decrypt shot
            var ssd = new ShotSyncData(new PangyaBinaryReader(_buffer));
        ...
        public GameBase(List<Player> _players, RoomInfoEx _ri, RateValue _rv, bool _channel_rookie)
        {
            ...
            // Inicializa a PCBangMascot persist rain next hole
        ...
        public void requestReadSyncShotData(Player _session, packet _packet, ref ShotSyncData _ssd)
        {
            try
            {
                ...
                //check size packet
                if (_packet.GetRemainingData.Length < 54 || _packet.GetRemainingData.Length > 54)
                    throw new exception($"[GameBase::requestReadSyncShotData][Error] DecryptShot" + (_packet.GetRemainingData.Length < 56 ? "is null" : _packet.GetRemainingData.Length > 56 ? "bad struct" : ""), ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.GAME,
                       7, 0));
                ...
                _session.m_pi.v_cei.ToList().ForEach(_el =>
{
    ...
    motion_item.Contains(_element)))
        ...
        // Set Flag Game and finish_game PCBangMascot
