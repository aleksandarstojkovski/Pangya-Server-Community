    ...
    public class LoginTask : IDisposable
    {
        ...
        public void sendCompleteData()
        {
            ...
            try
            {
              ...
              var cmd_sl = new  CmdServerList(TYPE_SERVER.MSN);  // waitable

                snmdb.NormalManagerDB.getInstance().add(0, cmd_sl, null, null);
                             
                if (cmd_sl.getException().getCodeError() != 0)
                    _smp.message_pool.getInstance().push(new message("[LoginTask::sendCompleteData][Line157] " + cmd_sl.getException().getFullMessageError(), type_msg.CL_FILE_LOG_AND_CONSOLE));
                                            
                if (cmd_sl.getServerList().Count > 0)//requesita o messenger server aqui, mas fiz bloco para melhorar o codigo!
                {
