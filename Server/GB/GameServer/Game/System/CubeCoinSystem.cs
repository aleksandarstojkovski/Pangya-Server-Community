...
namespace Pangya_GameServer.Game.System
{
    public class CubeCoinSystem
    {
        ...
        public static CoinCubeInHole getAllCoinCubeInHole(byte course_id, byte number_hole)
        {
            ...
            {
                _smp.message_pool.getInstance().push(new message("[CubeCoinSystem::CoinCubeInHole][Warning] invalid number hole(" + number_hole + ")", type_msg.CL_FILE_LOG_AND_CONSOLE));
                return cbih;
            }

            var course = MapSystem.getInstance().getMap(course_id);
            if (course == null)
            {
                _smp.message_pool.getInstance().push(new message("[CubeCoinSystem::CoinCubeInHole][Warning] Course=" + course_id + " not found in sMap.", type_msg.CL_FILE_LOG_AND_CONSOLE));
                return cbih;
            }
        ...
        private void Initialize()
        {
            ...
            snmdb.NormalManagerDB.getInstance().add(0, cmd_cci, null, null);
            ...
            if (m_course.Count == 0)
                _smp.message_pool.getInstance().push(new message("[CubeCoinSystem::Initialize][Warning] Not Loaded!", type_msg.CL_FILE_LOG_AND_CONSOLE));
        ...
        public class CourseCtx
        {
            ...
            public void set(uint typeid, bool active)
            {
                ...
                Parallel.For(1, 19, i =>
                {
                    ...
                    //{
                    //        var log = "[CubeCoinSystem::CourseCtx::initialize][Warning] INSERT INTO pangya.pangya_coin_cube([COURSE], [HOLE], [TIPO], [TIPO_LOCATION], [X], [Y], [Z]) VALUES("
                    //        + (course_id) + ", " + i + ", " + (itt.tipo) + ", " + (itt.flag_location) + ", "
                    //        + (itt.location.x) + ", " + (itt.location.y) + ", " + (itt.location.z) + ")";
                    //         _smp.message_pool.getInstance().push(new message(log));
                    //    }
                });
            }
            ...
            private void Initialize()
            {
                ...
                snmdb.NormalManagerDB.getInstance().add(0, cmd_ccli, null, null);

                if (cmd_ccli.getException().getCodeError() != 0u)
                    throw cmd_ccli.getException();

                m_coin_cube = cmd_ccli.getInfo();
            }
        }
