...
namespace Pangya_GameServer.Game.Manager
{
    ...
    {
        const uint _VERSION_PACKET_CALCULE = 0x12F; // 0x12 � o pacote que da para fazer o calculo em todos os modos, 0x12F s� pode fazer o calculo no tournament(base)
        ...
        protected void calculeShotCube(CalculeCoinCubeUpdateOrder _cccuo)
        {
          ...
          _cccuo.shot_data_for_cube.wind_influence.z);
                ...
                else if (_VERSION_PACKET_CALCULE == 0x12)
                {
                    float distance = (float)_cccuo.last_location.diffXZ(_cccuo.pin) * DIVIDE_SCALE_PANGYA;

                    Club3D club = new Club3D(sAllClubInfo3D.getInstance().m_clubs[_cccuo.shot_data_for_cube.taco], calculeTypeDistance(distance));
          ...
          //          var deg_360 = (uint)(_cccuo.shot_data_for_cube.wind_influence..degree / 255.0f * 360.0f);

          //          Vector3D wind = new Vector3D(_wind * (float)Math.Sin(deg_360 * PI / 180.0f) * -1.0f,
          //              0.0f,
          //              _wind * (float)Math.Cos(deg_360 * PI / 180.0f));

          //          var extra_power = _cccuo.shot_data_for_cube.getExtraPower();
