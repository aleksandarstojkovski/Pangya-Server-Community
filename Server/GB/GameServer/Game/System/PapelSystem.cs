...
#pragma warning restore CS0162 // Código inacessível detectado
        ...
        public List<ctx_papel_shop_ball> dropBalls(Player _session)
        {
            ...
            while (num_ball > 0)
            {
                ...
                // Player já tem o item, e nao pode ter duplicate, sortea um novo para ele
                if ((!sIff.getInstance().IsCanOverlapped(ctx_psi._typeid) || sIff.getInstance()._getItemGroupIdentify(ctx_psi._typeid) == PangyaAPI.IFF.GB.Models.Flags.IFF_GROUP.CAD_ITEM) && _session.m_pi.ownerItem(ctx_psi._typeid))
                {
                    continue;
                }
                // Color
