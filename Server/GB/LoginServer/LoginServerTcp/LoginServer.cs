   ...
   new Regex(@".*[\^$&,\\?`´~\|""@#¨'%*!\\].*", RegexOptions.Compiled);
        ...
        public LoginServer() : base(m_player_manager)
        {
            ...
            try
            {
                ...
                var auth_key_game = CommandDB.GetAuthKeyGame(14641, 20202);
            }
            catch (exception e)
            {
        ...
        protected override void onAcceptCompleted(PangyaAPI.Network.PangyaSession.Session _session)
        {
            try
            {
                ...
                var US = new byte[] { 0x00, 0x0B, 0x00, 0x00, 0x00, 0x00, _session.m_key, 0x00, 0x00, 0x00, 0x75, 0x27, 0x00, 0x00 };

                _session.m_client.Send(US, US.Length);
        ...
        public void requestLogin(Player _session, packet _packet)
        {
            ...
            try
            {
                ...
                if (result.password.Length < 2 || InvalidIdRegex.IsMatch(result.password))
                    throw new exception("[LoginServer::RequestLogin][Error] PASS(" + result.password
                            + ") invalid, less then 2 characters or invalid character include in pass.", (uint)STDA_ERROR_TYPE.LOGIN_SERVER);
                ...
                if (!haveBanList(_session.m_ip, result.mac_address, false))
                {   // Verifica se está na list de ips banidos

                    var _uid = CommandDB.VerifyID(result.id); // ID
                    ...
                    else if (!getAccessFlag() && getCreateUserFlag())
                    {
                        ...
                        _uid = (int)CommandDB.CreateUser(result.id, result.password, ip, getUID());
                        if (_uid > 0 )
                        {
                            if (CommandDB.CreateBetaUser((uint)_uid, result.id, result.password, ip, getUID()))
                               _smp.message_pool.getInstance().push("[LoginServer::RequestLogin][Log] Player[ID: " + result.id + ", IP: " + ip + "] Criado com sucesso.", type_msg.CL_FILE_LOG_AND_CONSOLE);
        ...
        public void requestDownPlayerOnGameServer(Player _session)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // Loga com sucesso
                    packet_func_ls.SUCCESS_LOGIN("login_server", this, _session);

                    packet_func_ls.pacote00F(_session);
                }
