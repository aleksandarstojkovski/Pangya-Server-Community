    ...
    public class mssql : database
    {
        ...
        private void executeReader(string proc)
        {
            ...
            using (var cmd = new OdbcCommand(proc, m_ctx_db._mssql.hDbc))
            {
                ...
                {
                    OdbcDataReaderEx readerEx = null;

                    // comportamento SqlClient-like
                    do
                    {
                        if (reader.FieldCount > 0)
                        {
                            readerEx = new OdbcDataReaderEx(reader);
                            break;
                        }
                    }
                    ...
                    for (int i = 0; i < readerEx.FieldCount; i++)
                        stmt.Columns.Add(readerEx.GetName(i));

                    while (readerEx.Read())
                    {
                        var row = new object[readerEx.FieldCount];
