...
namespace Pangya_GameServer.GameServerTcp
{
    public class GameServer : Server
    {
        ...
        private void init_Packets()
        {
            ...
            packet_func.funcs.addPacketCall(0x9A, packet_func.packet09A, this);
            ...
            packet_func.funcs.addPacketCall(0x173, packet_func.packet173, this);
            packet_func.funcs.addPacketCall(0x174, packet_func.packet174, this);
            packet_func.funcs.addPacketCall(0x175, packet_func.packet175, this);
            ...
            packet_func.funcs_sv.addPacketCall(0xAE, packet_func.packet_svFazNada, this);
            ...
            packet_func.funcs_sv.addPacketCall(tipo: 0x24B, packet_func.packet_svFazNada, this);
            packet_func.funcs_sv.addPacketCall(tipo: 0x122, packet_func.packet_svFazNada, this);
        ...
        public void init_load_channels()
        {
            ...
            try
            {
                ...
                for (byte i = 0; i < num_channel; ++i)
                {
                    ...
                    {
                        _smp.message_pool.getInstance().push(new message("[GameServer::init_load_channels][ErrorSystem] " + e.Message, type_msg.CL_FILE_LOG_AND_CONSOLE));
                    }
            ...
            "vai tomar no cu", "corna", "prostituta", "lixo"
                ...
                if (_session.ChatPenalty.IsStillBlocked())
                {
                    if (palavrasProibidas.Any(p => msg.IndexOf(p, StringComparison.OrdinalIgnoreCase) >= 0))
                    {
                        ...
                        // Log da tentativa
                        _smp.message_pool.getInstance().push(
                            new message($"[GameServer::requestChat][BLOCKED][Info]: PLAYER[UID: {_session.m_pi.uid}, MSG: {msg}]",
                            type_msg.CL_FILE_LOG_AND_CONSOLE));
        ...
        public void requestLogin(Player _session, packet _packet)
        {
            ...
            try
            {
                ...
                if (sClientType.getInstance().getType() == ClientType.eClientType.JP)
                { mac_address = _packet.ReadString(); }
                if (sClientType.getInstance().getType() == ClientType.eClientType.US)
                {
                    _smp.message_pool.getInstance().push(new message("[GameServer::requestLogin][Info] " + _packet.Log(), type_msg.CL_FILE_LOG_AND_CONSOLE));
                    _packet.Skip(8);
                }
                ...
                if (sClientType.getInstance().getType() == ClientType.eClientType.JP && this.haveBanList(_session.getIP(), mac_address, !mac_address.empty()))
                ...
                if (string.IsNullOrEmpty(_session.m_pi.id))
                {
                ...
                if (_session.m_pi.block_flag.m_id_state.ull_IDState != 0)
                {
                    ...
                    else if (_session.m_pi.block_flag.m_id_state.L_BLOCK_MAC_ADDRESS)
                    {
                        ...
                        snmdb.NormalManagerDB.getInstance().add(10, new CmdInsertBlockMac(mac_address), this.SQLDBResponse, this);
                ...
                var server_packet_version = this.getInfo().packet_version;
                //// Se a flag do canSameIDLogin estiver ativo, não verifica Packet
                if (!this.canSameIDLogin() && Packet_version != server_packet_version)// ver depois sobre isso(meu cliente)
                {
                ...
                if (!this.canSameIDLogin() && cmd_akli.getInfo().valid == 0)
            ...
            string.Compare(kol.keys[0], cmd_akli.getInfo().key) != 0
                ...
                ClientVersion cv_side_sv = ClientVersion.MakeVersion(this.getClientVersionSideServer());
                var cv_side_c = ClientVersion.MakeVersion(client_version);

                if (cv_side_c.flag == ClientVersion.COMPLETE_VERSION
    && string.Equals(cv_side_c.region, cv_side_sv.region)
    && string.Equals(cv_side_c.season, cv_side_sv.season))
                {

                    if (cv_side_c.high != cv_side_sv.high || cv_side_c.low < cv_side_sv.low)
                    {
                        _smp.message_pool.getInstance().push(new message("[GameServer::requestLogin][Warning] PLAYER[UID=" + (_session.m_pi.uid) + "].\tClient Version not match. Server: "
                                 + (this.getClientVersionSideServer()) + " == Client: " + cv_side_c.ToString(), type_msg.CL_FILE_LOG_AND_CONSOLE));

                        _session.m_pi.block_flag.m_flag.all_game = true;// |= BLOCK_PLAY_ALL;
                    }
                ...
                _session.setMemberInfo(cmd_mi.getInfo());
                ...
                if (_session.m_pi.m_cap.game_master)
                {
                    ...
                    _session.m_pi.mi.state_flag.visible = !_session.m_gi.visible; //inicia ligado!
                ...
                if (this.m_si.propriedade.only_rookie && _session.m_pi.level >= 6/*Beginner E maior*/)
                ...
                if (this.m_si.propriedade.mantle && !(_session.m_pi.m_cap.mantle || _session.m_pi.m_cap.game_master))
                ...
                var player_logado = this.HasLoggedWithOuterSocket(_session);

                if (player_logado != null)
                {
                    if (!this.canSameIDLogin())
                    {
                        _smp.message_pool.getInstance().push(new message("[GameServer::requestLogin][Error] PLAYER[UID=" + (_session.m_pi.uid) + ", OID="
                        ...
                        if (!this.DisconnectSession(player_logado))
                ...
                _session.m_pi.block_flag.m_flag.ullFlag |= this.m_si.flag.ullFlag;
                ...
                snmdb.NormalManagerDB.getInstance().add(5, new CmdRegisterLogon(_session.m_pi.uid, 0/*Logou*/), this.SQLDBResponse, this);

                // Resgistra o Login do Player no gs
                snmdb.NormalManagerDB.getInstance().add(7, new CmdRegisterLogonServer(_session.m_pi.uid, this.m_si.uid), this.SQLDBResponse, this);

                _smp.message_pool.getInstance().push(new message("[GameServer::requestLogin][Sucess] PLAYER[OID=" + (_session.m_oid) + ", UID=" + (_session.m_pi.uid) + ", NICK="
                         + (_session.m_pi.nickname) + ", Stage = Check].", type_msg.CL_FILE_LOG_AND_CONSOLE));
                ...
                //snmp.NormalManagerDB.getInstance().add(11, new CmdFirstAnniversary(), this.SQLDBResponse, this);  

                // Cria o login manager para carregar o cache das informações e itens completo do player
                m_login_manager.createTask(_session);

                // Time que check o TTL, para prevenção de Bots mal feitos
                _session.m_tick_bot = Environment.TickCount;

                // Entra com sucesso 
                packet_func.session_send(packet_func.pacote044(this.m_si, eLoginAck.ACK_AUTO_RECONNECT, _session.m_pi), _session, 0);
            }
            catch (exception ex)
            {
                _smp.message_pool.getInstance().push(new message(
              $"[GameServer::requestLogin][Error] {ex.getFullMessageError()}",
              type_msg.CL_FILE_LOG_AND_CONSOLE));
                ...
                this.DisconnectSession(_session);
            }
        }
